					...
					<tbody className='h-auto p-6 font-normal'>
						{
							search.length > 0 ?
								results
									.map((result, index) => (
										<tr className={`${index % 2 == 0 ? '' : 'bg-light-gray'}`} key={result.dentistId}>
											<td className='p-2 flex justify-center  '>
												<img src={result.profile} className=' w-11 h-11 rounded-full border-2 border-medium-gray' alt="User" />
											</td>
											...
											<td className=' text-center '>
												{result.verified ?
													<p className='px-2 py-2 rounded-md bg-green text-white cursor-pointer hover:shadow-md' onClick={() => disableAccountBtn(result.dentistId, false)}>Active</p> :
													<p className='px-2 py-2 rounded-md bg-red text-white cursor-pointer hover:shadow-md' onClick={() => disableAccountBtn(result.dentistId, true)}>Inactive</p>
												}
											</td>
											<td className=' h-auto relative bottom-2 w-auto flex items-start justify-center gap-3'>
												...
												<p className='px-5 py-2 rounded-md bg-medium-gray text-white cursor-pointer hover:shadow-md flex'><AiOutlineFolderView size={25} />&nbsp;View</p>
											</td>
										</tr>))
								: results
									//       firstItem         LastItem
