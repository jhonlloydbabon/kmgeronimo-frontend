...
function CovidServiceModal({ show, setModal, setAddModal, data }) {
  ...
  return (
    <div
      className={`w-full h-screen bg-modal bg-opacity-75 absolute top-0 z-40 flex flex-grow justify-center items-center ${show ? '' : 'hidden'}`}
    >
      <ToastContainer />
      <div className=' z-50 h-auto relative'>
        <div className='m-auto w-[900px] h-[750px] overflow-auto p-8 bg-gradient-to-t from-darkprimary to-subprimary  rounded-lg relative shadow-lg scrollbar scroll-smooth text-light-gray text-justify'>

          {/* Title */}
          <div className=' w-full p-4 flex flex-col justify-center items-center gap-y-4 relative '>
            <div className=' w-auto h-auto p-2 text-white rounded-full absolute top-0 left-0 bg-primary cursor-pointer ' onClick={backPrevious}>
          ...
          {/* Conditions */}
          {
            rules.map((rule, index) => (
              <div className=' w-full p-4  ' key={index}>
                <h1 className=' text-lg font-bold text-secondary '>{index + 1}. {rule.title}</h1>
                <p className=' text-secondary '>{rule.description}</p>
              </div>
            ))
          }

          <div className='w-full p-4 flex justify-end gap-3 '>
            <button className='px-6 py-4 rounded-md bg-primary text-secondary ' onClick={btnSubmit}>I accept the agreement</button>
            <button className='px-10 py-4 rounded-md bg-red text-secondary' onClick={() => setModal(false)}>Cancel</button>
          </div>
        </div>
      </div>
    </div>
